...
char *richiesta_stringa(){
	...
	while(reit==1){
		...
		//ACQUISIZIONE E CONTROLLI DELLA STRNGA INSERITA DALL'UTENTE
		...
		if(strlen(tmp)>50){//potrei scrivere semplicemente else perchè se \n è presente in tmp vuol dire che ci sono al massimo 50 caratteri
			printf("ERRORE 004: frase immessa troppo lunga\nReinseriscila per favore: ");
			while(getchar()!='\n');//assorbe tutti i caratteri rimasti nel buffer fino a quando non incontra \n
		}

		//strcpy(risp, tmp);
		else if(strspc(tmp)!=0){//la stringa inserita è vuota o composta solo da spazi e tabbature
			printf("ERRORE 002: frase immessa non valida\nReinseriscila per favore: ");
		}
		else{//la stringa è di max 50 caratteri e NON è vuota o formata solo da spazi e/o tabbature
			...
			for (i=0; i<strlen(tmp)-n; i++){//copia la stringa di tanti spazi indietro quanti sono gli spazi calcolati precedentemente
				tmp[i]=tmp[i+n];
			}
			tmp[strlen(tmp)-n]='\0';//pone un '\0' alla fine della stringa copiata
...
bool richiesta_si_no(){
	...
	while(reit==1){
		...
		if(strlen(tmp)>2){//potrei scrivere semplicemente else perchè se \n è presente in tmp vuol dire che ci sono al massimo 2 caratteri
			while(getchar()!='\n');//assorbe tutti i caratteri rimasti nel buffer fino a quando non incontra \n
			printf("ERRORE 001: comando immesso non valido\nReinseriscilo per favore: ");
		}
		else if(!strcmp(tmp, "NO") || !strcmp(tmp, "no")) return 0;
		else if(!strcmp(tmp, "SI") || !strcmp(tmp, "si")) return 1;
		else{
			printf("ERRORE 001: comando immesso non valido\nReinseriscilo per favore: ");
		}
	}
...
int richiesta_comando(int indice){
	...
	while(reit==1){
		...
		if(strlen(tmp)>8){//potrei scrivere semplicemente else perchè se \n è presente in tmp vuol dire che ci sono al massimo 8 caratteri
			while(getchar()!='\n');//assorbe tutti i caratteri rimasti nel buffer fino a quando non incontra \n
			printf("ERRORE 001: comando immesso non valido\nReinseriscilo per favore: ");
		}
		...
		else if(!strcmp(tmp, "COMANDI") || !strcmp(tmp, "comandi")) return 2;
		else if(!strcmp(tmp, "INDIETRO") || !strcmp(tmp, "indietro")) {
			if (indice==0){
				printf("ERRORE 003: impossibile tornare indietro alla prima domanda.\nInserisci un altro comando per favore: ");
			}
			else return 3;
		}
		else if(!strcmp(tmp, "SALVA") || !strcmp(tmp, "salva")) return 4;
		else if(!strcmp(tmp, "MENU") || !strcmp(tmp, "menu")) return 5;
		else if(!strcmp(tmp, "ESCI") || !strcmp(tmp, "esci")) return 6;
		else {
			printf("ERRORE 001: comando immesso non valido\nReinseriscilo per favore: ");
		}
	}
